...
func (s *application) ApplicationManagementPrivateItem(ctx context.Context, req *dto.ApplicationManagementItemRequest) (*entity.ApplicationPrivate, error) {
	...
	res, err := s.repository.ApplicationPrivateItem(ctx, &dto.ApplicationItemRequest{
		ApplicationID: req.ApplicationID,
	})
	if err != nil {
		if err := s.repository.HistoryNewEvent(ctx, userID, fmt.Sprintf("error get private application_id=%d", req.ApplicationID)); err != nil {
	...
	if res == nil {
		if err := s.repository.HistoryNewEvent(ctx, userID, fmt.Sprintf("get not exist private application_id=%d", req.ApplicationID)); err != nil {
			logger.Error.Printf("error writing history message: get not exist private application_id=%d", req.ApplicationID)
		}
		return nil, status.Error(codes.NotFound, "application not exist")
	}

	if err := s.repository.HistoryNewEvent(ctx, userID, fmt.Sprintf("get private application_id=%d", req.ApplicationID)); err != nil {
